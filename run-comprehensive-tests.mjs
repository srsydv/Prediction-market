...
const tempTestFile = join(__dirname, 'temp-test.mjs');
writeFileSync(tempTestFile, testScript);

console.log('🚀 Running comprehensive LMSRMarket tests...\n');
...
async function runComprehensiveTests() {
  ...
  try {
    ...
    console.log('\\n📝 Deploying contracts...');
    const MockERC20Factory = await ethers.getContractFactory('MockERC20');
    const usdc = await MockERC20Factory.deploy('USD Coin', 'USDC', 6, ethers.parseUnits('1000000', 6));
    console.log('✅ MockERC20 deployed at:', usdc.target);
    ...
    console.log('\\n🧪 TEST 1: Market Creation');
    console.log('-'.repeat(40));
    const tx = await lmsrMarket.connect(creator).createMarket(usdc.target, ethers.parseEther('1000'), ethers.parseUnits('1000', 6), 50);
    await tx.wait();
    console.log('✅ Market created successfully');
